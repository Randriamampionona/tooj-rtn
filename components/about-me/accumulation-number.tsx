...
const data = [
  {
    ...
    className: "-top-[2rem] right-[2.5rem]",
  },
  {
    id: "personal-project",
    num: 7,
    text: "personal projects",
    className: "-bottom-[1rem] right-[2rem]",
  },
  {
    id: "real-project",
    num: 5,
    text: "real-world projects",
    className: "bottom-[45%] -left-[1rem] sm:-left-[4rem]",
  },
];
    ...
    <div className="absolute flex items-center justify-start space-x-4 mb-2 w-[25rem] h-[25rem] ">
      {data.map((item) => (
        <div
          key={item.id}
          className={cn(
            "absolute flex items-center min-w-[7rem] max-w-[8.5rem] backdrop-blur space-x-2 border border-slate-100/20 bg-slate-100/20 dark:bg-transparent rounded p-2 select-none cursor-default",
            item.className
          )}
        >
